...
define("pagebody", "home");

online_other("arab-forums", "home", "0", "0", "0", "0");

echo bodytop_template("arab-forums", "الصفحة الرئيسية");
...
$online = array(0, 0, 0, 0, 0, 0, 0);

$sql_online = select_mysql("arab-forums", "online", "count(online_ip) , online_group", "group by online_group order by online_group asc");

while ($array_online = array_mysql("arab-forums", $sql_online)) {

    $online[$array_online[1]] = $array_online[0];
}

if (group_user == 0 || group_user == 1 || group_user == 2) {

    $totalonline = ($online[0] + $online[1] + $online[2]);
} elseif (group_user == 3) {

    $totalonline = ($online[0] + $online[1] + $online[2] + $online[3]);
} elseif (group_user == 4) {

    $totalonline = ($online[0] + $online[1] + $online[2] + $online[3] + $online[4]);
} elseif (group_user == 5) {

    $totalonline = ($online[0] + $online[1] + $online[2] + $online[3] + $online[4] + $online[5]);
} else {

    $totalonline = ($online[0] + $online[1] + $online[2] + $online[3] + $online[4] + $online[5] + $online[6]);
}

echo "<table cellpadding=\"7\" cellspacing=\"3\" width=\"99%\" align=\"center\"><tr>";

echo "<td class=\"stats\"><nobr>المتصلين : <span style=\"color:#801313;\">" . num_other("arab-forums", $totalonline) . "</span></nobr></td>";

echo "<td class=\"stats\"><nobr>زوار : <span style=\"color:#ff0000;\">" . num_other("arab-forums", $online[0]) . "</span></nobr></td>";

echo "<td class=\"stats\"><nobr>أعضاء : <span style=\"color:#ff0000;\">" . num_other("arab-forums", $online[1]) . "</span></nobr></td>";

echo "<td class=\"stats\"><nobr>مشرفين : <span style=\"color:#ff0000;\">" . num_other("arab-forums", $online[2]) . "</span></nobr></td>";

if (group_user > 2) {

    echo "<td class=\"stats\"><nobr>نواب مراقبين : <span style=\"color:#ff0000;\">" . num_other("arab-forums", $online[3]) . "</span></nobr></td>";
}

if (group_user > 3) {

    echo "<td class=\"stats\"><nobr>مراقبين : <span style=\"color:#ff0000;\">" . num_other("arab-forums", $online[4]) . "</span></nobr></td>";
}

if (group_user > 4) {

    echo "<td class=\"stats\"><nobr>مراقبين عامين : <span style=\"color:#ff0000;\">" . num_other("arab-forums", $online[5]) . "</span></nobr></td>";
}

if (group_user > 5) {

    echo "<td class=\"stats\"><nobr>مدراء : <span style=\"color:#ff0000;\">" . num_other("arab-forums", $online[6]) . "</span></nobr></td>";
}
...
echo "<table cellpadding=\"4\" cellspacing=\"1\" width=\"99%\" align=\"center\"><tr>";

echo "<td width=\"100%\"></td>";

echo "<td class=\"stats\"><nobr>" . times_date("arab-forums", "time", time()) . "</nobr></td>";

echo "<td><select class=\"inputselect\" onchange=\"relext(this)\">";

for ($x = -12; $x <= 12; $x++) {

    echo "<option value=\"change.php?go=time&value=" . ($x == 0 ? "00" : $x) . "\" " . (forum_time == $x ? "selected" : "") . ">GMT " . ($x == 0 ? "" : ($x > 0 ? "+{$x}" : $x)) . "</option>";
}
...
if (num_mysql("arab-forums", $style_sql) != false) {
    ...
    while ($style_object = object_mysql("arab-forums", $style_sql)) {
        ...
        echo "<option value=\"change.php?go=style&value={$style_object->style_fils}\" " . (forum_style == $style_object->style_fils ? "selected" : "") . ">{$style_object->style_name}</option>";
    }
}
...
$cat_sql = select_mysql("arab-forums", "cat", "u1.user_id as u1user_id , u1.user_lock1 as u1user_lock , u1.user_nameuser as u1user_name , u1.user_group as u1user_group , u1.user_coloruser as u1user_color , u2.user_id as u2user_id , u2.user_lock1 as u2user_lock , u2.user_nameuser as u2user_name , u2.user_group as u2user_group , u2.user_coloruser as u2user_color , c.cat_id , c.cat_lock , c.cat_hid , c.cat_name , c.cat_order , c.cat_home , c.cat_monitor1 , c.cat_monitor2 , c.cat_monitor1text , c.cat_monitor2text , c.cat_group" . group_user . "", "as c left join user" . prefix_connect . " as u1 on(u1.user_id = c.cat_monitor1) left join user" . prefix_connect . " as u2 on(u2.user_id = c.cat_monitor2) where c.cat_home in(0) && c.cat_group" . group_user . " in(1) order by c.cat_order asc , c.cat_id asc");
...
if (num_mysql("arab-forums", $cat_sql) != false) {
    ...
    while ($cat_object = object_mysql("arab-forums", $cat_sql)) {
        ...
        if ($cat_object->cat_hid == false || ($cat_object->cat_hid == true && cathide_other("arab-forums", $cat_object->cat_id, $cat_object->cat_monitor1, $cat_object->cat_monitor2) == true)) {
            ...
            echo "<td class=\"tcat\" width=\"40%\" colspan=\"2\" style=\"font-size:17px;\" align=\"right\"><div class=\"pad\">" . a_other("arab-forums", "cat.php?id={$cat_object->cat_id}", "{$cat_object->cat_name}", "{$cat_object->cat_name}", "") . "</div></td>";

            echo "<td class=\"tcat\" width=\"9%\"><div class=\"pad\">المواضيع</div></td>";

            echo "<td class=\"tcat\" width=\"9%\"><div class=\"pad\">الردود</div></td>";

            echo "<td class=\"tcat\" width=\"2%\"><div class=\"pad\">" . img_other("arab-forums", "images/user1.png", "المتواجدون في هذا المنتدى حاليا", "", "", "0", "class=\"title\"", "") . "</div></td>";

            echo "<td class=\"tcat\" width=\"12%\"><div class=\"pad\">آخر مشاركة</div></td>";

            echo "<td class=\"tcat\" width=\"28%\"><div class=\"pad\">المشرفون</div></td>";
                        ...
                        if ($forum_object->forum_hid2 == false || ($forum_object->forum_hid2 == true && forumhide2_other("arab-forums", $cat_object->cat_id, $cat_object->cat_monitor1, $cat_object->cat_monitor2, $forum_object->forum_mode) == true)) {
                            ...
                            if ($forum_object->forum_moderattext == 1) {
                                ...
                                if ($forum_object->forum_mode > 0) {
                                    echo "<span style=\"font-size:11px;\">مجموعة " . $group_list[$forum_object->forum_mode] . "</span>";
                                }
