...
function page1_forum($id, $table, $all, $url1)
{
    ...
    if ($paging > 0) {
        if ($page > 1) {
            $first = '<td class="thead"><nobr><a href="' . $url1 . '1">البداية</a></nobr></td>';
            $prev = '<td class="thead"><nobr><a href="' . $url1 . '' . ($page - 1) . '">السابق</a></nobr></td>';
        } else {
            $first = '';
            $prev = '';
        }
        $check = '<table width="100%" cellpadding="0" cellspacing="1" align="center"><tr><td width="100%"></td><td><table class="tborder" cellpadding="10" cellspacing="2" width="100%" align="center"><tr align="center"><td class="tcat"><nobr>الصفحة ' . $page . ' من ' . $paging . '</nobr></td>' . $first . $prev . '';
        if ($page > 1) {
            $equal = $page - ($page == 2 ? 1 : 2);
            for ($x = $equal; $x < $page; $x++) {
                $check .= '<td class="thead"><nobr><a href="' . $url1 . '' . $x . '">' . $x . '</a></nobr></td>';
            }
        }
        $check .= '<td class="tcat"><nobr>' . $page . '</nobr></td>';
        if ($page < $paging) {
            $equal = $page + ($page == ($paging - 1) ? 1 : 2);
            for ($x = $page + 1; $x <= $equal; $x++) {
                $check .= '<td class="thead"><nobr><a href="' . $url1 . '' . $x . '">' . $x . '</a></nobr></td>';
            }
            $next = '<td class="thead"><nobr><a href="' . $url1 . '' . ($page + 1) . '">التالي</a></nobr></td>';
            $last = '<td class="thead"><nobr><a href="' . $url1 . '' . $paging . '">الأخيرة</a></nobr></td>';
        } else {
            $next = '';
            $last = '';
        }
        $check .= '' . $next . $last . '</tr></table></td></tr></table>';
    }
