...
function ip_other($copi)
{
    if ($copi == "arab-forums") {
        ...
        if (!empty($_SERVER[$ip_ip1])) {
            $ip_check = $_SERVER[$ip_ip1];
        } elseif (!empty($_SERVER[$ip_ip2])) {
            $ip_check = $_SERVER[$ip_ip2];
        } else {
            $ip_check = $_SERVER[$ip_ip3] ?? '0.0.0.0';
        }

        // في حالة السيرفر ما أعطاش IP، رجّع قيمة افتراضية
        if (empty($ip_check)) {
            $ip_check = @getenv($ip_ip1) ?: (@getenv($ip_ip2) ?: (@getenv($ip_ip3) ?: '0.0.0.0'));
        }
...
function confirm_other($copi, $msg)
{
    ...
    if ($copi == "arab-forums") {
        ...
        if ($msg == "") {
            $msg = "هل أنت متأكد من أنك تريد إدخال البيانات الجديدة ؟";
        }
