...
if ($error != "") {
    ...
    online_other("arab-forums", "monitor", "0", "0", "0", "0");
    ...
    online_other("arab-forums", "monitor", "0", "0", "0", $user_object->user_id);

    if ($user_object->user_id == id_user) {

        $usertitle = "قائمة مواضيع مفضلتك";

        $usertitleerror = "لا توجد أي مواضيع لك بمفضلتك حاليا";

        $urtl = "monitor.php";
    } else {

        $usertitle = "قائمة مواضيع مفضلة " . ($user_object->user_sex == 1 ? "العضو" : "العضوة") . " " . $user_object->user_nameuser . "";

        $usertitleerror = "لا توجد مواضيع بمفضلة " . ($user_object->user_sex == 1 ? "العضو" : "العضوة") . " " . $user_object->user_nameuser . " حاليا";

        $urtl = "monitor.php?id={$user_object->user_id}";
    }
    ...
    echo "<td>" . img_other("arab-forums", "images/monitor.png", "", "", "", "0", "", "") . "</td>";

    echo "<td width=\"100%\">" . a_other("arab-forums", "monitor.php?id={$user_object->user_id}", "{$usertitle}", "{$usertitle}", "") . "</td>";
    ...
    echo "<td class=\"tcat\" width=\"9%\"><div class=\"pad\">تاريخ الإضافة</div></td>";

    echo "<td class=\"tcat\" width=\"15%\"><div class=\"pad\">المنتدى</div></td>";

    echo "</tr>";

    $topic_sql = select_mysql("arab-forums", "monitortopic", "i.iconstopic_id , i.iconstopic_name , i.iconstopic_images , i.iconstopic_forumid , x.texttopic_id , x.texttopic_name , x.texttopic_forumid , m.monitortopic_id , m.monitortopic_topicid , m.monitortopic_userid , m.monitortopic_date , c.cat_id , c.cat_lock , c.cat_hid , c.cat_name , c.cat_monitor1 , c.cat_monitor2 , c.cat_group" . group_user . " , f.forum_id , f.forum_catid , f.forum_lock , f.forum_hid1 , f.forum_hid2 , f.forum_name , f.forum_logo , f.forum_moderattext , f.forum_totaltopic , f.forum_sex , f.forum_group" . group_user . " , f.forum_mode , u1.user_id as u1user_id , u1.user_lock1 as u1user_lock , u1.user_nameuser as u1user_name , u1.user_group as u1user_group , u1.user_coloruser as u1user_color , u2.user_id as u2user_id , u2.user_lock1 as u2user_lock , u2.user_nameuser as u2user_name , u2.user_group as u2user_group , u2.user_coloruser as u2user_color , t.topic_id , t.topic_forumid , t.topic_lock , t.topic_name , t.topic_wait , t.topic_delete , t.topic_hid , t.topic_stiky , t.topic_top , t.topic_survey , t.topic_icons , t.topic_text , t.topic_reply , t.topic_visit , t.topic_date , t.topic_user , t.topic_lastdate , t.topic_lastuser", "as m left join topic" . prefix_connect . " as t on(t.topic_id = m.monitortopic_topicid) left join iconstopic" . prefix_connect . " as i on(i.iconstopic_id = t.topic_icons) left join texttopic" . prefix_connect . " as x on(x.texttopic_id = t.topic_text) left join forum" . prefix_connect . " as f on(f.forum_id = t.topic_forumid) left join cat" . prefix_connect . " as c on(c.cat_id = f.forum_catid) left join user" . prefix_connect . " as u1 on(u1.user_id = t.topic_user) left join user" . prefix_connect . " as u2 on(u2.user_id = t.topic_lastuser) where m.monitortopic_userid in({$user_object->user_id}) && t.topic_delete in(0) && c.cat_group" . group_user . " in(1) && f.forum_group" . group_user . " in(1) order by m.monitortopic_date desc");
                ...
                if ($topic_object->forum_hid2 == false || ($topic_object->forum_hid2 == true && forumhide2_other("arab-forums", $topic_object->cat_id, $topic_object->cat_monitor1, $topic_object->cat_monitor2, $topic_object->forum_mode) == true)) {
                    ...
                    if ((($topic_object->topic_wait == 0) || ($topic_object->topic_wait == 1 && ($moderatget1 == true || $topic_object->topic_user == id_user))) && (($topic_object->topic_hid == 0) || ($topic_object->topic_hid == 1 && ($moderatget1 == true || $topic_object->topic_user == id_user || num_mysql("arab-forums", select_mysql("arab-forums", "hidtopic", "hidtopic_userid , hidtopic_topicid", "where hidtopic_userid in(" . id_user . ") && hidtopic_topicid in({$topic_object->topic_id}) limit 1")) != false)))) {
                        ...
                        if (group_user > 0) {
                            ...
                            echo "<td>" . a_other("arab-forums", "monitordelete.php?id={$topic_object->topic_id}", "إزالة الموضوع من قائمة المواضيع المفضلة", img_other("arab-forums", "images/cut.png", "", "", "", "0", "", ""), confirm_other("arab-forums", "هل أنت متأكد من أنك تريد إزالة الموضوع من قائمة المواضيع المفضلة ؟")) . "</td>";
                        ...
                        echo "<td class=\"topic\"><nobr>" . times_date("arab-forums", "", $topic_object->monitortopic_date) . "</nobr></td>";
