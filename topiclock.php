...
$error = "للأسف خاصية المواضيع المغلوقة المفتوحة لك متوفرة للأعضاء المسجلين فقط";
...
online_other("arab-forums" , "topiclock" , "0" , "0" , "0" , "0");
...
online_other("arab-forums" , "topiclock" , "0" , "0" , "0" , $user_object->user_id);

if($user_object->user_id == id_user){

$usertitle = "قائمة المواضيع المغلوقة المفتوحة لك";

$usertitleerror = "لا توجد أي مواضيع مغلوقة مفتوحة لك";

$urtl = "topiclock.php";

}else{

$usertitle = "قائمة المواضيع المغلوقة المفتوحة ".($user_object->user_sex == 1 ? "للعضو" : "للعضوة")." ".$user_object->user_nameuser."";

$usertitleerror = "لا توجد أي مواضيع مغلوقة مفتوحة ".($user_object->user_sex == 1 ? "للعضو" : "للعضوة")." ".$user_object->user_nameuser." حاليا";

$urtl = "topiclock.php?id={$user_object->user_id}";
...
echo "<td>".img_other("arab-forums" , "images/topiclock.png" , "" , "" , "" , "0" , "" , "")."</td>";
...
$topic_sql = select_mysql("arab-forums" , "locktopic" , "i.iconstopic_id , i.iconstopic_name , i.iconstopic_images , i.iconstopic_forumid , x.texttopic_id , x.texttopic_name , x.texttopic_forumid , m.locktopic_id , m.locktopic_topicid , m.locktopic_userid , m.locktopic_date , c.cat_id , c.cat_lock , c.cat_hid , c.cat_name , c.cat_monitor1 , c.cat_monitor2 , c.cat_group".group_user." , f.forum_id , f.forum_catid , f.forum_lock , f.forum_hid1 , f.forum_hid2 , f.forum_name , f.forum_logo , f.forum_moderattext , f.forum_totaltopic , f.forum_sex , f.forum_group".group_user." , f.forum_mode , u1.user_id as u1user_id , u1.user_lock1 as u1user_lock , u1.user_nameuser as u1user_name , u1.user_group as u1user_group , u1.user_coloruser as u1user_color , u2.user_id as u2user_id , u2.user_lock1 as u2user_lock , u2.user_nameuser as u2user_name , u2.user_group as u2user_group , u2.user_coloruser as u2user_color , t.topic_id , t.topic_forumid , t.topic_lock , t.topic_name , t.topic_wait , t.topic_delete , t.topic_hid , t.topic_stiky , t.topic_top , t.topic_survey , t.topic_icons , t.topic_text , t.topic_reply , t.topic_visit , t.topic_date , t.topic_user , t.topic_lastdate , t.topic_lastuser" , "as m left join topic".prefix_connect." as t on(t.topic_id = m.locktopic_topicid) left join iconstopic".prefix_connect." as i on(i.iconstopic_id = t.topic_icons) left join texttopic".prefix_connect." as x on(x.texttopic_id = t.topic_text) left join forum".prefix_connect." as f on(f.forum_id = t.topic_forumid) left join cat".prefix_connect." as c on(c.cat_id = f.forum_catid) left join user".prefix_connect." as u1 on(u1.user_id = t.topic_user) left join user".prefix_connect." as u2 on(u2.user_id = t.topic_lastuser) where m.locktopic_userid in({$user_object->user_id}) && t.topic_delete in(0) && c.cat_group".group_user." in(1) && f.forum_group".group_user." in(1) order by m.locktopic_date desc");
...
if(($moderatget1 == true) || ($topic_object->topic_wait == 0)){
...
echo "<td class=\"topic\"><nobr>".times_date("arab-forums" , "" , $topic_object->locktopic_date)."</nobr></td>";
