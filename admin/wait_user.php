...
if (isset($wait)) {
    ...
    if ($allyu == 0) {
        ...
        update_mysql("arab-forums", "user", "user_wait = \"0\" where user_id in({$import})");
    ...
    if ($allyu == 0) {
        ...
        $arraymsg = array(
            ...
            "msg" => "الرجاء تحديد عضوية وآحدة على الأقل ليتم رفضها",
        ...
        update_mysql("arab-forums", "user", "user_wait = \"0\" , user_bad = \"1\" where user_id in({$import})");

        $arraymsg = array(

            "msg" => "تم رفض العضويات المحددة بنجآح تام",
    ...
    echo page_pager("arab-forums", "user", "user_id , user_wait , user_active", "where user_wait in(1) && user_active in(0)", $count_page, $get_page, "admin.php?gert=wait&go=wait_user&");
    ...
    echo "<td><nobr><input class=\"button\" value=\"رفض العضويات المحددة\" type=\"submit\" name=\"bad\" " . confirm_other("arab-forums", "هل أنت متأكد من أنك تريد رفض العضويات المحددة ؟") . "></nobr></td>";
    ...
    $inputtext = array(
        ...
        3 => "لا يوجد عضويات بالصفحة حاليا",
    ...
    $user_sql = select_mysql("arab-forums", "user", "user_id , user_wait , user_active , user_dateregister , user_nameuser , user_adressip , user_email , user_country", "where user_wait in(1) && user_active in(0) order by user_dateregister asc limit {$limit_page},{$count_page}");
    ...
    if (num_mysql("arab-forums", $user_sql) != false) {
        ...
        echo "<td class=\"alttext1\" align=\"center\" colspan=\"6\"><br><div class=\"pad\">لا توجد أي عضوية تنتظر الموافقة حاليا</div><br></td>";
